...
public class SectorView extends View{
    ...
    private static final int                         mSlashLineLength   = 45;           //直线和斜线添加的长度
    private static       int            mTextPaintColor;

    private int[]                       mAngelePerStatus;                  //每个状态占用的数量,状态分别是失联、打开、正常、低电
    private int[]                       mAngeleSort;                       //每种角度大小排序排第几位，比如失联状态的数据占第一位

    private boolean                     mDataNull = true;                          //数据是否为空
    ...
    private void init(Context context, AttributeSet attrs){
        ...
        mColorPerStatus[0] = typedArray.getColor(R.styleable.sector_status1color, mDefaultLostColor);
        mColorPerStatus[1] = typedArray.getColor(R.styleable.sector_status2color, mDefaultOpenColor);
        mColorPerStatus[2] = typedArray.getColor(R.styleable.sector_status3color, mDefaultNormalColor);
        mColorPerStatus[3] = typedArray.getColor(R.styleable.sector_status4color, mDefaultLowColor);
        mStatusInfo[0] = typedArray.getString(R.styleable.sector_status1text);
        mStatusInfo[1] = typedArray.getString(R.styleable.sector_status2text);
        mStatusInfo[2] = typedArray.getString(R.styleable.sector_status3text);
        mStatusInfo[3] = typedArray.getString(R.styleable.sector_status4text);
        mTextPaintColor = typedArray.getColor(R.styleable.sector_text_color, mDefaultLowColor);
        typedArray.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(!mDataNull){
            ...
            canvas.drawText("暂无数据", (int) mCircle.x/1.1f, mCircle.y, mTextPaint);
        }
    ...
    private void drawLineAndText(Canvas canvas){
        ...
        for(int i = 0; i < mAngelePerStatus.length; i++){
            ...
            int slashLineEndX = mCircle.x + mCircle.radiusLevel[currentAngleLevel] + mSlashLineLength + mSlashLineLength;
            ...
            if(rotateAngle > 90 && rotateAngle < 270){
                ...
                horizonLineEndX = mCircle.x + mCircle.radiusLevel[currentAngleLevel] - mHorizonLineLength/2;
                ...
                horizonLineEndX = mCircle.x + mCircle.radiusLevel[currentAngleLevel] + mHorizonLineLength+120;
                if(horizonLineEndX > mViewWidth){                                       //防止超出右边界
                    horizonLineEndX = mViewWidth;
                }
                textStartX = slashLineEndX + 10;
            }
